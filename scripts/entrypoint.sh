...
then
  ...
  kvd init ${MONIKER:-nonamenode} --home=${KVD_HOME:-/.kvd} --chain-id=${CHAIN_ID:-kava-2}
...
[program:kvcli]
command=kvcli rest-server --laddr tcp://0.0.0.0:${LCD_PORT:-1317} --home=${KVD_HOME:-/.kvd} --trust-node --node=${RPC_LADDR:-tcp://0.0.0.0}:${RPC_PORT:-26657}
