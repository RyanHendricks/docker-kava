...
then
  ...
  kvd init ${MONIKER:-nonamenode} --home=${KVD_HOME:-/.kvd} --chain-id=${CHAIN_ID:-kava-3}
  ...
  if [ ! -z "$GENESIS_URL" ]; then
      ...
      wget https://raw.githubusercontent.com/Kava-Labs/launch/master/kava-3/genesis.json
  fi
...
[program:kvcli]
command=kvcli rest-server --laddr tcp://0.0.0.0:${LCD_PORT:-1317} --home=${KVD_HOME:-/.kvd} --chain-id=${CHAIN_ID:-kava-3} --trust-node --node=${RPC_LADDR:-tcp://0.0.0.0}:${RPC_PORT:-26657}
